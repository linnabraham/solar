...
class aia_euv(Dataset):
    ...
    def __getitem__(self, idx):
        item = self.data[idx]
        #features = [self._read_fits_file(item[str(i)]) for i in range(7)]
        features = [self._read_fits_file(item[str(i)]) for i in [2,5,0,1,3,4,6]]
...
class CustomTransform:
    def __init__(self, means=None, stds=None, zscore=False):
        if zscore:
